...
- Till here only show an object with end poing friends, an HTML page with message endpoint and 404 if the endpoint does not match
...
1. Pipe re request back to the response"
   req.on('data', (data)=>{
   const friend = data.toString();
   console.log('request:', friend);
   friends.push(JSON.parse(friend));
